...
**Status:** completed through the staged execution recorded in `UPSTREAM_SYNC.md`
...
| **Knowledge base on Intel** (incl. project knowledge collections) | **COMPLETED 2026-09-08.** The local collection registry, encrypted index, management UI, tools, and project grants are now compiled on Intel. | Implemented as its own post-`1.0.34` arc, using the pure-Swift model2vec path shared with Memory. |
